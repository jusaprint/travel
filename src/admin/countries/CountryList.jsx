...
import { getCountries, deleteCountry } from '../services/dataService';
  ...
  useEffect(() => {
    const loadCountries = async () => {
      try {
        ...
        setMessage({ type: 'error', text: 'Failed to load countries. Please try again.' });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleDeleteCountry = async (id) => {
    if (window.confirm('Are you sure you want to delete this country? This action cannot be undone.')) {
      try {
        ...
        setMessage({ type: 'error', text: 'Failed to delete country. Please try again.' });
      }
    }
  };
      ...
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manage countries and their associated packages
          </p>
        </div>
        <Link
        ...
        <>
                      ...
                      {filteredCountries.map((country) => (
                        <tr key={country.id}>
                          ...
                          <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            <Link to={`/admin/countries/${country.id}`} className="text-[#690d89] hover:text-[#8B5CF6] mr-4">
                              Edit
                            </Link>
                            <button 
          ...
          {filteredCountries.length === 0 && (
            <div className="mt-8 text-center py-12 bg-gray-50 rounded-lg">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {searchTerm || statusFilter !== 'all' || regionFilter !== 'all' ? 'Try adjusting your search or filters.' : 'Get started by adding a country.'}
              </p>
